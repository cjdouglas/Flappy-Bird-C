...
void renderer_init(renderer_t* self, bird_t* bird, pipes_t** pipes,
                   size_t n_pipes) {
  self->shader =
...
void renderer_draw(renderer_t* self) {
  ...
  for (size_t i = 0; i < self->n_pipes; ++i) {
    shader_uniform_mat4(self->shader, "model",
                        (float*)self->pipes[i]->model_bottom);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
    shader_uniform_mat4(self->shader, "model",
                        (float*)self->pipes[i]->model_top);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
  }
